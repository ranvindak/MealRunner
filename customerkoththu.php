...
<head><link rel="stylesheet" type="text/css" href="add.css"/>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Customer Koththu</title>
...
<br>
 ...
 <div id="contl">   <!--cont div left start-->  
            <h3 id="topp">Koththu List For You</h3>
            <div>
            	 <?php
	include"connect.php";
	
	$sql="SELECT * FROM koththu";
	...
	<tr>
		...
		<td><b>Koththu Name</b></td>
		<td><b>Koththu Large Price</b></td>
		<td></td>
		<td><b>Koththu Small Price</b></td>
		
	</tr>';
		while($row=mysqli_fetch_assoc($result))
		{
			
			 echo'<tr>'.'<td>'.$row['K_name'].'</td>'.'<td>'.$row['K_Lprice'].'<td>'.'<button onclick="location.href='."'koththuLorder.php ? Id=".$row['K_id']."'".'">Order now</button>'.'</td>'.'</td><td>'.$row['K_Sprice'].
'<td><button onclick="location.href='."'koththuSorder.php ? Id=".$row['K_id']."'".'">Order Now</button></td>'.'</td>'.'</tr>';
