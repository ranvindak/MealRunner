...
<br>
 ...
 <div id="contl">   <!--cont div left start-->  
            <h3 id="topp">Fried Rice List For You</h3>
            <div>
  	<?php
	include"connect.php";
	...
	<tr>
		...
		while($row=mysqli_fetch_assoc($result))
		{
			 ...
			 echo'<tr>'.'<td>'.$row['R_name'].'</td>'.'<td>'.$row['Lprice'].'<td>'.
			 ' <button onclick="location.href='."'riceLorder.php ? Id=".$row['R_id']."'".'">Order Now</button></td></td><td>'.$row['Sprice'].'<td><button onclick="location.href='."'ricesorder.php ? Id=".$row['R_id']."'".'">Order Now</button></td></tds>'.'<td>'.'</td>'.'</tr>';
